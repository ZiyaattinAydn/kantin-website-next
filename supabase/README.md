...
## Kritik admin kayıtları sürüm geçmişi

`migrations/20260625010000_admin_record_revisions.sql`

Bu migration `branches`, `site_settings`, `site_pages` ve `content_blocks` kayıtlarının ekleme, güncelleme ve silme öncesi/sonrası hâllerini aynı transaction içinde `admin_record_revisions` tablosuna yazar. Mevcut içerikleri değiştirmez ve geçmiş işlemleri geriye dönük oluşturmaz.

Migration sonrasında şu test ve doğrulama dosyaları kullanılmalıdır:

- `supabase/tests/admin_record_revisions.test.sql`
- `supabase/verification/verify_admin_record_revisions.sql`

Tam rollback yalnız bilinçli olarak `supabase/manual/rollback_admin_record_revisions.sql` ile yapılmalıdır; bu işlem birikmiş snapshot geçmişini kalıcı olarak siler.

## Kritik kayıt sürüm geri yükleme

`migrations/20260625020000_admin_record_revision_restore.sql`

Bu migration, kritik kayıtların `update` snapshot'larını yönetim panelinden geri yüklemek için admin-only transaction RPC'si ekler. Geri yükleme yalnız normalde düzenlenebilen alanları etkiler; sistem kimlikleri korunur ve işlem yeni snapshot olarak tekrar kaydedilir.
