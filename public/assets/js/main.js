(() => {
  ...
  const setNavState = (open, { restoreFocus = false } = {}) => {
    ...
    const hiddenOnMobile = !open && window.innerWidth < 1024;
    ...
    if (open) {
      ...
      requestAnimationFrame(() => {
        const target = lastFocusedElement instanceof HTMLElement ? lastFocusedElement : toggle;
        target?.focus();
      });
    }
  };
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (event.key !== "Tab" || toggle?.getAttribute("aria-expanded") !== "true" || !nav) return;
