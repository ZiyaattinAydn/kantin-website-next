...
import type { MemoriesSectionData } from "@/lib/public-data/types";
...
export default function HomeMemories({
  section = fallbackHomeData.memoriesSection,
  photos = fallbackHomeData.memoryPhotos,
}: {
  section?: MemoriesSectionData;
  photos?: MemoryPhoto[];
}) {
      ...
      <div className={`container ${styles.container}`}>
        ...
        <div className={styles.storyGrid}>
          <figure className={`${styles.statementPhoto} reveal`}>
            <button
              aria-label={`${photos[0].caption} fotoğrafını büyüt`}
              className={styles.photoButton}
              onClick={(event) => openLightbox(photos[0], event.currentTarget)}
              type="button"
            >
              <Image
                fill
