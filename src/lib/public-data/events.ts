...
async function loadEventPublicData(): Promise<
PublicDataEnvelope<EventPublicData>
...
const eventRows =
  (eventsResult.data ?? []) as unknown as EventPublicRow[];
...
const mediaRows = await getPublicMediaRows(
  client,
  mediaIds,
);
...
const branchIdsByEvent = new Map<
  string,
  string[]
>();
...
for (const link of linksResult.data ?? []) {
  const currentIds =
    branchIdsByEvent.get(link.event_id) ?? [];

  currentIds.push(link.branch_id);

  branchIdsByEvent.set(
    link.event_id,
    currentIds,
  );
}

const rawEvents: RawEvent[] = eventRows.map(
  (event) => {
    const branchSlugs = (
      branchIdsByEvent.get(event.id) ?? []
    )
      .map(
        (branchId) =>
          branchByUuid.get(branchId)?.slug,
      )
      .filter(
        (slug): slug is string =>
          Boolean(slug),
    ...
    return {
      ...
      link:
        event.external_url ?? undefined,

      ctaLabel:
        event.cta_label ?? undefined,
      ...
      publishStartAt:
        event.publish_start_at,

      publishEndAt:
        event.publish_end_at,

      sortOrder:
        event.sort_order ?? undefined,
    };
  },
);
...
const events = normalisePublishedEvents(rawEvents).map(
(event) => {
const createdAt =
typeof event.id === "string"
? createdAtById.get(event.id)
: undefined;
...
return {
  data: {
    ...
    ...maps,
    instagramUrl:
      fallbackCommonData.siteIdentity
        .instagramUrl,
  },
...
export const getEventPublicData = cache(
loadEventPublicData,
);
