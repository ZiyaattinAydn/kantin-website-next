...
import { formatTryPriceInput, isUuid } from "@/lib/admin/pricing";
import {
...
export default async function AdminPricingPage({ searchParams }: Props) {
  ...
  const activeFilter = firstString(params.active) ?? "all";
  ...
  if (productIds.length) {
    const { data, error } = await supabase
      ...
      .select(
        "id, menu_item_id, branch_id, price_cents, price_label, price_note, availability_note, is_active, updated_at",
      )
    ...
    <section className={adminStyles.page}>
      <header className={adminStyles.head}>
        <div>
          ...
          <p>
            Yiyecek ve içecekleri ayırmadan, ürünün bütün şube fiyatlarını ve varyantlarını tek akışta güncelle.
          </p>
        </div>
        <div className={adminStyles.actions}>
          <Link className={adminStyles.secondary} href="/admin/manage/menu-items">Ürün içerikleri</Link>
          <Link className={adminStyles.secondary} href="/admin/manage/menu-item-branches">Gelişmiş şube tablosu</Link>
          <Link className={adminStyles.secondary} href="/admin/manage/menu-item-variants">Gelişmiş varyant tablosu</Link>
        </div>
      </header>
            ...
            <article className={styles.productCard} key={product.id}>
              <header className={styles.productHead}>
                <div>
                  ...
                  <h2>{product.name}</h2>
                  <p>{product.slug} · Son ürün güncellemesi {formatAdminDate(product.updated_at)}</p>
                </div>
                <Link href={`/admin/manage/menu-items?edit=${product.id}`}>İçeriği düzenle →</Link>
              </header>
                    ...
                    <section className={styles.branchCard} key={branch.id}>
                      <div className={styles.branchHead}>
                        ...
                        {branchPrice ? (
                          ...
                          <small className={styles.missing}>Bağlantı yok</small>
                        )}
                      </div>

                      {branchPrice ? (
                        <>
                          <form action={updateAdminBranchPrice} className={styles.priceForm}>
                            <input name="id" type="hidden" value={branchPrice.id} />
                            <input name="return_to" type="hidden" value={returnTo} />
                            <label>
                              <span>Temel fiyat (TL)</span>
                              <input
                                aria-label={`${product.name} ${branch.name} temel fiyatı`}
                            ...
                            <label>
                              ...
                              <input defaultValue={branchPrice.price_label ?? ""} maxLength={120} name="price_label" placeholder="Porsiyon / Başlangıç" />
                            </label>
                            <label className={styles.activeToggle}>
                              <input defaultChecked={branchPrice.is_active} name="is_active" type="checkbox" />
                            ...
                            <details className={styles.notes}>
                              ...
                              <label>
                                ...
                                <input defaultValue={branchPrice.price_note ?? ""} maxLength={300} name="price_note" />
                              </label>
                              <label>
                                <span>Bulunabilirlik notu</span>
                                <input defaultValue={branchPrice.availability_note ?? ""} maxLength={300} name="availability_note" />
                              </label>
                            </details>
                            <button className={adminStyles.primary} type="submit">Şube fiyatını kaydet</button>
                          </form>

                          <div className={styles.variants}>
                            <div className={styles.variantsHead}>
                              <strong>Varyantlar</strong>
                              <Link href="/admin/manage/menu-item-variants?new=1">Yeni / gelişmiş yönetim →</Link>
                            </div>
                            {branchVariants.length ? branchVariants.map((variant) => (
                              <form action={updateAdminVariantPrice} className={styles.variantRow} key={variant.id}>
                                ...
                                <label className={styles.variantActive}>
                                  <input defaultChecked={variant.is_active} name="is_active" type="checkbox" />
                                  <span>Aktif</span>
                                </label>
                                <input defaultValue={variant.price_note ?? ""} maxLength={300} name="price_note" placeholder="Fiyat notu" />
                                <button className={adminStyles.secondary} type="submit">Kaydet</button>
                              </form>
                            )) : (
                        ...
                        <form action={createAdminBranchPrice} className={styles.priceForm}>
                          <input name="menu_item_id" type="hidden" value={product.id} />
                          <input name="branch_id" type="hidden" value={branch.id} />
                          <input name="return_to" type="hidden" value={returnTo} />
                          <p className={styles.missingText}>Bu ürün henüz {branch.name} şubesine bağlanmamış.</p>
                          <label>
                            <span>Temel fiyat (TL)</span>
                            <input inputMode="decimal" name="price" placeholder="85,50" />
                          </label>
                          <label>
                            <span>Fiyat etiketi</span>
                            <input maxLength={120} name="price_label" placeholder="Porsiyon / Başlangıç" />
                          </label>
                          <label className={styles.activeToggle}>
                            <input defaultChecked name="is_active" type="checkbox" />
                            <span>Bu şubede aktif</span>
                          </label>
                          <button className={adminStyles.primary} type="submit">Şubeye ekle</button>
                        </form>
                      )}
                    </section>
                  );
